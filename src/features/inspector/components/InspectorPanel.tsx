...
const InspectorPanel = () => {
  ...
  const moduleInfo = useMemo(
    () => buildInspectorModule(selectedGraphNode, activeGraphNode, activeFile, graphEdges),
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-auto p-4 sm:p-5">
        {!moduleInfo ? (
          <div className="mx-auto flex w-full max-w-[340px] flex-col items-center justify-center rounded-3xl border border-dashed border-[#334155] bg-[#111827] p-6 text-center">
            <p className="text-sm font-semibold text-white">No module selected</p>
            <p className="mt-2 text-xs text-[#94A3B8]">
              Select a graph node or source file to see summary details and dependency metadata.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <InspectorSection title="Overview" description="Basic module metadata and selection source.">
              <div className="space-y-2">
                <div className="rounded-2xl bg-[#0F172A] p-3">
                  <p className="text-sm font-semibold text-white">{moduleInfo.name}</p>
                  <p className="truncate text-xs text-[#94A3B8]">{moduleInfo.path}</p>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl bg-[#0F172A] p-3">
                    <p className="text-[11px] uppercase tracking-[0.22em] text-[#94A3B8]">File type</p>
                    <p className="mt-2 text-sm font-semibold text-white">{moduleInfo.fileType}</p>
                  </div>
                  <div className="rounded-2xl bg-[#0F172A] p-3">
                    <p className="text-[11px] uppercase tracking-[0.22em] text-[#94A3B8]">Source</p>
                    <p className="mt-2 text-sm font-semibold text-white">
                      {moduleInfo.source === 'graph' ? 'Dependency graph' : 'File selection'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <InspectorSection title="Imports" description="Modules imported by the selected module.">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl bg-[#0F172A] p-3">
                  <p className="text-[11px] uppercase tracking-[0.22em] text-[#94A3B8]">Import count</p>
                  <p className="mt-2 text-lg font-semibold text-cyan-300">{moduleInfo.importCount}</p>
                </div>
                <div className="rounded-2xl bg-[#0F172A] p-3">
                  <p className="text-[11px] uppercase tracking-[0.22em] text-[#94A3B8]">Imported by</p>
                  <p className="mt-2 text-lg font-semibold text-cyan-300">{moduleInfo.exportCount}</p>
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#94A3B8]">Imported modules</p>
                {moduleInfo.importedModules.length > 0 ? (
                  <ul className="space-y-2">
                    {moduleInfo.importedModules.map((item) => (
                      <li key={item} className="rounded-2xl bg-[#0F172A] px-3 py-2 text-sm text-[#E2E8F0]">
                        {item}
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-xs text-[#94A3B8]">No outgoing dependency records found.</p>
                )}
              </div>
            </InspectorSection>

            <InspectorSection title="Dependencies" description="Modules that depend on the selected module.">
