...
const GraphNode: FC<NodeProps<GraphNodeData>> = ({ data, selected }) => {
  const fileType = data.fileType ?? data.path.split('.').pop()?.toUpperCase() ?? 'FILE'

  return (
    <div
      className={`min-h-[84px] min-w-[200px] max-w-[280px] rounded-3xl border bg-[#111827] p-4 shadow-[0_24px_60px_-36px_rgba(15,23,42,0.9)] transition-all duration-150 ${
        selected
    ...
    >
      <div className="flex items-center justify-between gap-3 text-left">
        ...
        <span className="shrink-0 rounded-full border border-slate-700 bg-slate-900 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-300">
          {fileType}
        </span>
      </div>
      {data.metadata ? (
        <div className="mt-3 rounded-2xl bg-[#0F172A]/80 px-3 py-2 text-xs text-slate-400">
